; css-emu-bios.asm — Emulated BIOS for CSS-DOS
...
; ============================================================
int13h_handler:
    ; Hard disk (DL >= 0x80): no hard drives present, return error for all calls.
    ; Without this, the kernel thinks floppy params are for a hard disk and
    ; builds a corrupt DDSC chain that loops forever during config init.
    cmp dl, 0x80
    jae .no_drive
    cmp ah, 0x00
    je .disk_reset
    ...
    je .disk_change
.no_drive:
    ; Unknown function or hard disk — return error
    mov ah, 0x01           ; invalid function / no such drive
    stc
    iret

.disk_change:
    ; AH=16h: Disk change status — not changed
    xor ah, ah
    clc
    iret
...
.disk_read:
    ...
    ; Copy CX sectors * 512 bytes using REP MOVSW
    ; DS:SI = source (disk image), ES:DI = destination (caller buffer)
    cld
.read_sector_loop:
    push cx
    mov cx, 256             ; 256 words = 512 bytes per sector
    rep movsw               ; DS:SI -> ES:DI, CX words
    ; Advance DS by 32 paragraphs (512 bytes) for next sector
    mov cx, ds
    add cx, 0x20
    mov ds, cx
    xor si, si              ; SI back to 0 (start of new segment)
    pop cx
    dec cx
