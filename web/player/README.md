...
| `raw.html` | The "theoretical" player. Mirrors `calcite.html`'s chrome exactly (it's **derived** from it by `web/scripts/raw-regen.mjs`) but replaces the `<img>` screen with a 320×200 = 64,000-element CSS pixel grid (`<i id=pN>`) and loads `/cabinet.css` as a real `<link rel="stylesheet">`. The cabinet's `kiln/pixels.mjs` rules paint each pixel from the Mode 13h framebuffer via `@function --paletteRGB()` over the live DAC — spec-correct for a compliant CSS evaluator. In practice Chrome crashes/hangs on the cabinet's (and grid's) sheer size; that's the point. **Known bug (2026-07-09 FINDING):** the same-element `class="clock cpu"` lets `.cpu`'s animation shorthand cascade-clobber the clock's `anim-play`, so the machine is frozen at tick 0 by construction — fix is nesting `.clock > .cpu` in raw-regen. Measured behavior: parse ~6 s, first tick ~5 min, then a permanent recalc backlog (see `experiments/huge-css-test.html`). Regenerate with `node web/scripts/raw-regen.mjs`. |
