...
<div class="subpage">
  ...
  <ol class="cheat-list">
    <li>
      <b>Calcite must produce EXACTLY what a spec-compliant browser would - byte for byte.</b> This is the &lsquo;cardinal rule&rsquo;, front and centre in the documentation. Feed the same cabinet to Chrome&rsquo;s own style engine and you&rsquo;d get the exact same behaviour, just unbearably slowly. I have not allowed ONE BYTE of divergence in my conformance testing - no fast approximations, no cut corners or hacking. If Calcite disagrees with a spec-compliant browser, that&rsquo;s a bug in Calcite. The CSS IS the source code, and Calcite is its servant - perfect, or nothing.
    </li>
    <li>
      <b>Calcite must parse a .css file on-the-spot and blind - no AOT compilation, no pre-baking.</b> It compiles whatever arrives at load time, in the browser, the same way V8 takes whatever JavaScript arrives: everything inferred from the file itself, immediately. And an even stricter rule: it must be the same file. Not one byte added, changed, or removed to make Calcite&rsquo;s life easier - it can only interpret and recreate.
    </li>
    <li>
      <b>The player page must contain ZERO JavaScript - only HTML/CSS.</b> Calcite lives in a separately-loaded service worker, mimicking an in-browser engine like V8, and the screen is fed back to the tab through an HTML element. I could easily support the real keyboard, improve performance, etc. with JavaScript on the player page. But it wouldn&rsquo;t be right.
    </li>
    <li>
      <b>Calcite must be generic - no knowledge of x86, DOS, or this repo.</b> It doesn&rsquo;t know it&rsquo;s compiling a CPU, and it never finds out what the program does - it only ever reasons about the shape of the CSS. Since CSS has so few tools, the shapes are enumerable in advance even with no idea what they&rsquo;re for. Point it at a different computational stylesheet - another CPU, a Pong cabinet, a cellular automaton, a spreadsheet encoded in selectors - and it would speed those up just the same.
    </li>
    <li>
