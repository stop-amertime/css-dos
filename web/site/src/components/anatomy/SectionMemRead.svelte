...
<h3 class="anatomy-head">Why are all three in one function?</h3>
<p>
  At first it might seem strange to cram RAM, BIOS ROM and the hard
  disk into one read function. Why not three functions, and pick the
  right one wherever a read happens?
</p>
<p>
  Because we can&rsquo;t know which one we&rsquo;ll need. The address
  being read isn&rsquo;t written down anywhere in the program &mdash;
  it&rsquo;s computed on the spot, usually from registers: <i>fetch me
  whatever DS &times; 16 + BX points at</i>. The exact same MOV
  instruction reads RAM this tick, a ROM table the next, the disk
  window the one after &mdash; wherever its registers happen to be
  pointing. Any read can land anywhere. So one function has to answer
  for everywhere.
</p>
...
<p>
  Instead, they&rsquo;re a <i>window</i>. The BIOS writes the number of
  the sector it wants into an agreed memory cell (<code>--mc632</code>),
  then reads these 512 addresses. Each arm computes <i>the requested
  sector &times; 512 + its own position in the window</i> &mdash; the
  first arm serves the sector&rsquo;s first byte, the last one its
  512th &mdash; and hands the question straight to the disk function.
  Same 512 addresses, whichever sector was last asked for: change the
  number in the cell, and the view changes. (The gibberish in the
  formula &mdash; <code>mod(&hellip;, 256) + round(down, &hellip;)
  &times; 256</code> &mdash; is just gluing the 16-bit sector number
  back together from its packed cell.) The
  <a href="#about/file/disk">disk section</a> picks it up from there.
</p>

<h3 class="anatomy-head">If all else fails &mdash; 1</h3>
<p>
  736,510 + 6,924 + 512 + 2 = 743,948. Then <code>else: 0);</code>, and
  the function ends.
</p>
<p>
  It looks like boilerplate. It isn&rsquo;t &mdash; this arm answers
  <i>constantly</i>: every zero byte the ROM section left out lands
  here, and so does any address no arm mentions &mdash; the gaps
  between regions, places where nothing was ever built. A normal
  emulator would spend a bounds check and an error path on those. Here,
  a read of nowhere falls through all 743,948 questions and comes back
  0.
</p>
