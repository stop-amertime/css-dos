...
<p>
  CSS can&rsquo;t open anything at runtime - no files, no requests, no loading - so whatever the machine will ever need has to be in the stylesheet before it starts: the <Term t="bios">BIOS</Term>, <Term t="dos">DOS</Term> itself, and the entire <Term t="floppy">floppy disk</Term>, baked in byte by byte. The concept here is relatively simple - every byte of the floppy gets its own arm in one giant <code>if()</code>, and a function reads them back:
</p>
