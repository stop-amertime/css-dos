...
<p>
  <code>mod()</code> digs the pixel&rsquo;s byte out of its packed memory cell, and <code>--screenPx()</code> - the mode dispatcher - decides what the bytes mean. <code>--vidMode</code> is the machine&rsquo;s current video mode: in the 256-colour game mode it hands <code>--ci</code> to the palette function below; in text mode it reads the character cell instead and looks the glyph row up in the ROM font; in CGA it unpacks two-bit pixels. One rule, every screen the machine can show.
</p>

<SectionHead>64,000 divs? Pixel artists manage with one</SectionHead>
<p>
  True - the classic trick gives a single element thousands of <code>box-shadow</code>s, one per pixel. Could the screen be one div?
</p>
<p>
  Possibly! But the grid isn&rsquo;t the hot path - Calcite paints straight from video memory and never evaluates these rules - so it was never tested or optimised after scaling up from small experiments. There isn&rsquo;t even a way to paint all 64,000 at full speed to compare.
</p>

<SectionHead>The palette - how 256 colours get chosen</SectionHead>
<p>
...
<p>
  The mess inside <code>rgb()</code> is three live memory reads - red, green, blue - each scaled by 255/63 because a real VGA&rsquo;s palette chip only kept six bits per channel: programs wrote brightnesses from 0 to 63, and the machine honours that.
</p>
<p class="dim small">
  Of the file&rsquo;s thousands of functions, only three return a colour - this one, its fixed 16-colour cousin <code>--vgaRGB</code> (text and CGA modes), and the <code>--screenPx</code> dispatcher itself. All the others just return integers.
</p>
<Foldable>
  {#snippet summary()}The palette read-back cursor{/snippet}
  <p>
    There&rsquo;s a second, separate cursor for <i>reading</i> the palette back - a fade effect needs to read the current colours before dimming them, and real VGA hardware let it ask without disturbing the write cursor. If real hardware did it, we have to support it too.
  </p>
</Foldable>
...
<SectionHead>Text mode &amp; CGA - the shared bytes</SectionHead>
<p>
  <Term t="mode13h">Mode 13h</Term> isn&rsquo;t the only screen the machine carries. Text mode - the 80&times;25 grid the DOS prompt lives on - is its own region of video memory at a different address: two bytes per character, the letter and its colours. And the older CGA graphics modes have their own aperture which <b>overlaps the text region</b>. Yikes. That&rsquo;s how 1981 CGA hardware behaved, since RAM was scarce and you can&rsquo;t use both modes at once. We must mimic it faithfully.
</p>
<p>
  The overlap is authentic; what comes next isn&rsquo;t. On a real PC, a program&rsquo;s <code>OUT 0x3D9, AL</code> lands in a register <i>inside</i> the CGA chip, and the same chip uses it as it paints - the hardware that receives the setting is the hardware that draws. CSS-DOS has no such chip, and its screen is drawn by something else entirely: a separate renderer that reads the finished video memory. So the machine leaves that renderer a note. On the <code>OUT</code>, it copies the value into a spare byte of the BIOS data area, and does the same for the current video mode - two shadow bytes recording what the program <i>asked for</i>, so whatever draws the screen can tell which of these overlapping modes it meant. Both readers use it: the fast external renderer, and the 64,000 pixel rules above, whose <code>--vidMode</code> and <code>--vidPal</code> are exactly these two bytes. A real PC needs no such note, because there the sender and the painter are one piece of silicon.
</p>
<p>
  That palette byte carries one famous bit: the choice between CGA&rsquo;s two four-colour palettes - green/red/yellow or cyan/magenta/white - the reason so many old PC games are those exact colours.
</p>

<SectionHead>The electron beam</SectionHead>
<p>
  An 80s monitor painted the picture with an electron beam, top to bottom, 70 times a second - and games wait for the beam&rsquo;s flyback (the <i>vertical retrace</i>) to redraw without tearing. They poll a status port for this information.
</p>
<p>
