...
<p>
  For efficiency, only the first one is declared. The other three have no <code>@property</code> declaration &mdash; an unregistered CSS variable is simply created if it didn't already exist. They <i>do</i> need a power-on value for the very first tick, expressed as fallback as follows:
</p>
