...
assert.ok(css.includes('===== DISPLAY ====='), 'painter block emitted');
assert.ok(css.includes('@function --paletteRGB'), 'palette function emitted');
assert.ok(css.includes('@function --fontRow'), 'font function emitted');
assert.ok(css.includes('@function --screenPx'), 'mode dispatch function emitted');
assert.ok(/\.motherboard #p63999 \{/.test(css), 'full 320x200 grid emitted (last pixel)');
rmSync(out, { force: true });
rmSync(out.replace(/\.css$/, '.meta.json'), { force: true });
