...
Cart manifest at `program.json` (see `docs/cart-format.md`). Corduroy
(C BIOS at `bios/corduroy/`) is the default for DOS carts; Muslin
(assembly BIOS at `bios/muslin/muslin.asm`) remains as a fallback.
No microcode BIOS, no opcode 0xD6 dispatch.
...
**Active investigation: eliza / COMMAND.COM "keyboard doesn't work" (session
12).** See the session 12 entry for full details. Start point for a fresh
agent: run the minimal key-echo test (already built at
`AppData/Local/Temp/echo_test/ECHO.COM`, cart at `/tmp/echo_test.css`) and
see whether INT 21h AH=01h actually returns a key in corduroy. That one
test will either eliminate "keyboard broken" from suspicion or confirm
it. Eliza's observed failure is a stuck linked-list walk at
CS:IP=0FC8:0BE0 in the Watcom runtime (not its own code); RESPONSE.DAT
contents aren't in memory. Hypothesis: file-read fails or parses to
empty, eliza hangs before any input call, so "keyboard" is a misdiagnosis.
Unverified.
...
### Calcite repo
- Session 12 added a new `/run-until` endpoint to `calcite-debugger`
  (crates/calcite-debugger/src/main.rs). Accepts a condition object and
  runs forward until it matches or max_ticks elapse. Supported conditions:
  `cs_ip`, `cs`, `ip_range`, `int` (any INT), `int_num`, `property_equals`,
  `property_changes`, `mem_byte_equals`. Useful for finding specific
  execution points without polling. Tested: `int_num: 33` correctly locates
  INT 21h calls. Uncommitted.
...
186 instructions; user was skeptical. This session triaged whether that theory
was correct, then started debugging what actually fails. Did not reach a fix.

**Status:** unresolved. Investigation interrupted before verifying the core
hypothesis (that eliza hangs pre-input during data-structure init). Next
session should pick up where this one stopped.
...
- **Not a 186/286/386 opcode gap.** Disassembled ELIZA.EXE and unpacked
  ...
  opcodes. No 0x0F-prefix reachable code. No 0x66/0x67 prefixes.
  kernel.sys also checked; its few superficially-186-looking disasm hits
  are inside data tables (includes bogus MMX and RDTSC decodes, i.e.
  disassembler sync loss, not real opcodes).

- **Not a `--IP` overflow in the CSS.** Saw `state.registers['IP']`
  reporting values > 0xFFFF (e.g. 0x1698F) at tick 1,050,000. But the
  `properties` block at the same tick reports masked `--CS`/`--IP` and
  `--ipAddr` correctly inside the expected code segment. The overflow is
  in the debugger's `/state.registers` view (reads the raw state-var
  storage), not in instruction fetch. Cosmetic / debugger-reporting issue
  only; not relevant to the keyboard bug.

**What was observed (with evidence) but not root-caused:**

- **Eliza prints its prompt, then freezes at CS:IP = 0FC8:0BE0 cycling
  through five addresses (0BE0 → 0BE3 → 0BE7 → 0BCE → 0BD2 → 0BE0, period
  5 ticks).** At every tick in this loop, SI=0, ES=0x7B06, and the bytes
  at ES:SI (linear 0x7B060) are zero. Loop body disassembles as:
  ```
  0BCE  cmp dx, [es:si+6]
  0BD2  jnz 0xBE0
  0BD4  cmp ax, [es:si+8]
  ...
  0BDA  cmp bl, [es:si+4]
  0BDE  jz 0xC22
  0BE0  mov si, [es:si]        ; SI = next node
  0BE3  cmp si, [0x1E]          ; end marker check
  0BE7  jnz 0xBCE
  ```
  This is a linked-list walk looking for a match. `[ES:SI]` is 0, so SI
  stays 0 forever. Register state captured: AX=0x97D2 BX=0x0C04 CX=0xFF04
  DX=0xFD0D SS=0x0BAB SP=0x08F0 BP=0x08E2 DS=0x088E ES=0x7B06.
  cycleCount advances per tick; halt=0; flags bit 9 (IF) is 0.

- **Eliza is in the Watcom runtime segment, not its own image.** Scanning
  memory at tick 7,650,000 for the eliza entry signature
  (9A ?? ?? ?? ?? 55 89 E5 81 EC 00 01) found it at linear 0xDD9D with
  `CALL FAR 0DF2:0000`. So eliza's own image starts at seg 0x0DF2. The
  stuck code at seg 0x0FC8 is the Watcom runtime (matches the `0x88:0x0000`
...
- **RESPONSE.DAT contents are NOT in memory.** Scanning all addressable
  ...
  loaded / parsed / copied anywhere reachable.
...
**Current working hypothesis (not verified):**

Eliza hangs before any input call. The sequence seems to be:
1. Prints "Hi! I'm Eliza..." prompt (confirmed on screen).
2. Tries to open + read RESPONSE.DAT.
3. Something goes wrong (file read fails silently, or the parsed data
   ends up as an empty linked list).
4. Enters the linked-list walk in the Watcom runtime, finds all-zero
   nodes, loops forever.

"Keyboard doesn't work" is likely a **misdiagnosis**. The program
probably never reaches an INT 21h input call.

**This hypothesis is UNVERIFIED.** What's needed to verify or refute:

1. A minimal key-echo .COM test program under corduroy (20 bytes:
   print 'X', loop INT 21h AH=01, ESC to exit). If keys work → CSS
   keyboard / DOS input path is fine, eliza has its own failure. If keys
   don't work → there IS a CSS-side keyboard bug, distinct from the
...
2. Log the sequence of INT 21h calls eliza makes from boot onward,
   ...
   then find the AH=3Fh read calls and check their results.

3. Compare CSS eliza vs a reference emulator (`conformance/peripherals.mjs`
   or similar) running the same cart. First divergence = bug location.
   Not attempted.

**Infrastructure added this session:**

- `calcite-debugger` has a new endpoint `/run-until` accepting a
  condition: `cs_ip`, `cs`, `ip_range`, `int` (any), `int_num`,
  `property_equals`, `property_changes`, or `mem_byte_equals`. Returns
  tick, CS:IP, opcode, AH, and full register state on hit. Faster than
...
- Spent too long on byte-counting / disassembly of raw EXE files before
  ...
  answering but took longer than it should have.
- Multiple times the debugger process orphaned (left running in
  background, holding port 3334 / eating multiple GB RAM) across agent
  turns. Two suspected causes: (a) using `&` in Bash calls instead of
  the harness's `run_in_background: true` parameter, so processes died
  when the turn ended; (b) possibly multiple debugger instances from
  other concurrent agents. Netstat / tasklist checks were helpful
  recovery but should have been preemptive.
- Chased a red herring (`--IP` appearing > 0xFFFF in debugger output)
  before noticing it's the raw state-var storage value, not what's used
  for fetch. Cosmetic.
- The biggest miss: the first thing to do was the minimal key-echo
  test. Instead I went deep into disassembly and memory forensics. A
  fresh agent should do the echo test first and only go deeper if it
  doesn't immediately answer the keyboard-works-or-not question.
...
- Whether COMMAND.COM has the same stuck-loop behaviour as eliza, or a
  different failure mode. The logbook (session 11d) already noted
  COMMAND.COM fails separately ("Bad or missing command interpreter" on
  Corduroy, reaches prompt but isn't usable on Muslin). It may be a
  distinct bug and isn't necessarily the same keyboard issue.
- Whether Muslin (not Corduroy) shows the same eliza behaviour.
- Whether bootle's INT 21h usage differs from eliza's in a way that
  would explain why one works and the other doesn't. Bootle is a .COM
  file loaded as a single 64K segment; eliza is a multi-segment .EXE
